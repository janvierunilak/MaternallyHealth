...
export default function SingleWomen() {
    ...
    const generatePDF = (child) => {
        ...
        html2canvas(certificateContent).then((canvas) => {
            ...
            const certificateWidth = canvas.width * 0.264583; // convert canvas width from px to mm
            const certificateHeight = canvas.height * 0.264583; // convert canvas height from px to mm
            doc.addImage(imgData, "PNG", 0, 0, certificateWidth, certificateHeight);
            doc.save(`${child.firstName} ${child.lastName} Birth Certificate.pdf`);
        });
    };

    const printCertificate = (child) => {
        setSelectedChild(child);
        setShowModal(true);
        setTimeout(() => {
            generatePDF(selectedChild);
        }, 1000); // Wait for the modal to show up
    };
    ...
    const handleChildSelect = (child) => {
        const hospitalId = child.hosptital_id;
        setSelectedChild(child);
        setShowModal(true);
        const hospital = hospitals.find((hosp) => hosp.id === hospitalId);
        const hospitalName = hospital ? hospital.hospitalname : "";
                                                            ...
                                                            <div className="table-responsive">
                                                                        ...
                                                                        {filteredChildren.map((child) => (
                                                                            <tr key={child.id}>
                                                                                ...
                                                                                <td className="text-center">
                                                                                    <button className="b-primary" onClick={() => { handleChildSelect(child) }}>
                                                                ...
                                                                {selectedChild && (
                                                                    ...
                                                                    >
                                                                        <Modal.Header closeButton>
                                                                            <Modal.Title>
                                                                                {`Birth Certificate for ${selectedChild.firstName} ${selectedChild.lastName}`}
                                                                            </Modal.Title>
                                                                        </Modal.Header>
                                                                        <Modal.Body>
                                                                            <Certificate
                                                                        ...
                                                                        <Modal.Footer>
                                                                            <Button
                                                                                variant="secondary"
                                                                                onClick={() => printCertificate(selectedChild)}
                                                                            >
                                                                                Print
                                                                            </Button>
                                                                        </Modal.Footer>
